...
def process(step_folder):
    ...
    try:
        # Load cad data
        step_folder = Path(step_folder)
        if step_folder.suffix == '.step':
            step_path = step_folder 
            process_furniture = True
        else:
            files = list(step_folder.glob('*'))
            assert len(files) == 1 
            step_path = files[0]
            process_furniture = False

        # Check single solid
        cad_solid = load_step(str(step_path))
        if len(cad_solid)!=1: 
        ...
        if process_furniture:
            data_uid = f"{step_path.parent.name}_{step_path.name}"
            sub_folder = step_path.parent.parent.name
        else:
            data_uid = step_path.parent.name
        ...
        data['uid'] = data_uid
        OUTPUT = Path('abc_parsed')
        save_folder = OUTPUT / sub_folder
        save_folder.mkdir(parents=True, exist_ok=True)

        save_path = save_folder / f"{data['uid']}.pkl"
...
if __name__ == '__main__':
    ...
    if args.option == 'deepcad': 
        OUTPUT = Path('deepcad_parsed')
    elif args.option == 'abc': 
        OUTPUT = Path('abc_parsed')
    else:
        OUTPUT = Path('furniture_parsed')
