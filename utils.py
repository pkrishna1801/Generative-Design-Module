...
# And later in the code, perhaps in joint_optimize function:
...
def joint_optimize(surf_ncs, edge_ncs, surfPos, unique_vertices, EdgeVertexAdj, FaceEdgeAdj, num_edge, num_surf):
    ...
    for iters in range(200):
        ...
        for surf_pnt, edge_pnts in zip(surf_updated, face_edges):
            ...
            surf_loss += cpu_chamfer_distance(surf_pnt.unsqueeze(0), edge_pnts.unsqueeze(0), loss_func, bidirectional=False, reverse=True)
...
def cpu_chamfer_distance(x, y, loss_func, bidirectional=False, reverse=False):
    """Run chamferdist on CPU while preserving gradients"""
    # Store original device
    device = x.device
    
    # Move to CPU without detaching
    x_cpu = x.cpu()
    y_cpu = y.cpu()
    
    # Calculate distance on CPU
    loss = loss_func(x_cpu, y_cpu, bidirectional=bidirectional, reverse=reverse)
